...
export const DeleteConfirmModal: React.FC<Props> = ({ isOpen, onClose, onConfirm, itemName }) => {
      ...
      <div className="premium-card w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-500 border border-white/10">
        <div className="p-8 md:p-12 text-center">
          <div className="w-16 h-16 md:w-24 md:h-24 bg-rose-500 text-white rounded-2xl md:rounded-[32px] flex items-center justify-center mx-auto mb-6 md:mb-8 shadow-2xl shadow-rose-500/30 border-4 border-white/20">
            <Trash2 size={32} className="md:w-12 md:h-12" />
          </div>
          <h2 className="text-2xl md:text-4xl font-black text-brand-dark dark:text-brand-light mb-3 md:mb-4 tracking-tighter">Remove Item?</h2>
          <p className="text-brand-brown dark:text-brand-tan/80 mb-8 md:mb-12 leading-relaxed font-bold text-base md:text-lg">
            Are you sure you want to delete <span className="text-brand-dark dark:text-brand-light font-black underline decoration-rose-500/30 underline-offset-4">"{itemName}"</span>? This action is permanent.
          </p>
